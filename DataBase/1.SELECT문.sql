...
-- 정의된 데이터베이스에 입력된 레코드를 조회, 수정, 삭제 하는등의 역할을 하는언어
-- DDL(create, drop, alter, truncate)
-- DCL(Data Control Language, 권한을 부여받고 부여하는 조작어) -Grant, revate
-- TCL : COMMIT, rollback
...
-- SELECT ~ FROM ~ #무조건 있어야 함
SELECT * FROM member; # *은 모든것이라는 의미(파이썬과 똑같음)
-- SELECT : 테이블에 데이터를 가져올 때 사용하는 예약어
...
FROM member
-- WHERE 163 <= height <= 165; -- 이거 안됨(163-165 범위가 아닌 값들도 들어가있음)
...
SELECT *
FROM buy
#WHERE prod_name = '지갑' OR prod_name = '청바지'; -- 내 답안(정답은 맞음)
WHERE prod_name IN ('지갑', '청바지'); -- 강사님 답안
...
-- 평균 키가 164이상인 회원들을 키의 내림차순으로 조회할 때
SELECT mem_id, mem_name, debut_date, height
FROM member
WHERE height >= 164
ORDER BY height DESC;

-- 문자열 자료형의 데이터 대소관계는 사저식 순서에 의해 결정
SELECT mem_id, mem_name
FROM member
ORDER BY mem_name ASC; -- 한글(가 ~ 힣)
...
-- 정렬 기준을 여러 열로 지정하기(이차 정렬 기준)
-- > 데이터 양이 많은 경우 하나의 열만으로는 행을 특정짓기 어려운 때가 많음
-- > ORDER BY로 정렬할 때 같은 값을 가진 행의 순서는 데이터베이스 서버의 상황에 따라 어떤 행을 반환할 지 결정하여 일정하지 않음
SELECT mem_id, mem_name, debut_date, height
FROM member
WHERE height >= 164
...
-- NULL 값의 정렬순서
-- > NULL은 대소비교를 할 수 없어서 각 DMBS마다 기준이 다르지만 가장 작은 값 또는 가장 큰 값으로 처리
...
FROM member
ORDER BY debut_date ASC
LIMIT 2, 3; -- LIMIT 다음에 오는 최대 갯수는 2개

-- 중복 제거(DISTINCT) : 연산작업을 많이 해서 용량을 많이 잡아먹음
SELECT addr
FROM member;
...
-- GROUP BY(보통 집계함수랑 자주 쓰임)
-- > 그룹으로 묶어주는 역할
-- > 예) 각 회원이 구매한 물품의 총 갯수를 알고 싶은 경우
...
FROM - WHERE - GROUP BY - HAVING - SELECT - ORDER BY(별칭쓰기가능) - LIMIT


-- 연습문제
-- 1. member 테이블에서 회원들을 평균 키의 오름차순으로 조회하기
-- SELECT mem_name, mem_id, height (내 정답)
-- FROM member
...
-- SELECT *
-- FROM aac_intakes
-- WHERE name is NULL;(아무것도 안나옴)
-- WHERE name != '';(이름이 없는것들이 나옴)
...
SELECT YEAR(debut_date), MONTH(debut_date), DAY(debut_date), debut_date
FROM member;
