...
-- > 일대다 관계 : 회원 테이블의 아이디에는 하나의 값이 한 번만 등장해야하지만 다른 테이블에서는 여러 개의 값이 존재할 수 있는 관계
-- -- > 예) 회원테이블에서 회원아이디는 한 번만 등장하지만, 구매테이블에서는 한 아이디를 여러 번 찾을 수 있음

-- 기본키 : 테이블에서 유일한 값으로, 각 행의 데이터를 유일하게 확인할 수 있는 값
-- 외래키 : 두 테이블을 서로 연결하는 데 사용하는 키.
-- -- 왜래키가 포함된 테이블을 자식 테이블, 외래키값을 제공하는 테이블을 부모테이블이라고 함

-- 내부조인(회사에서 주로 쓰이는 건 내부조인)
...
-- 코드가 너무 길어지는 문제를 별칭으로 해결(가장 권장되는 코드 형태)
...
-- 천재지변으로 인해 데이터가 유실되었음.
-- 입양을 간 기록은 있는데, 보호소에 들어온 기록이 없는 동물의 id와 이름을 id의 오름차순으로 조회하기
-- 조회할 열 : animal_id, name, outcomes의 datetime, intakes의 datetime

-- 내가 작성한 코드(오답)
SELECT ai.animal_id, ai.name, ao.datetime 보호소기록, ai.datetime 입양기록
FROM aac_intakes ai
	LEFT OUTER JOIN aac_outcomes ao 
	ON ai.animal_id = ao.animal_id
ORDER BY ai.animal_id, ai.name ASC; 

-- 강사님 코드(정답)
SELECT outs.animal_id, outs.name, outs.datetime 보호소기록, ins.datetime 입양기록
