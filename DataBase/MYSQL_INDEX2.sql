...
-- -- 회원 이름은 중복이 있을 수도 있기 때문에 UNIQUE를 지정하면 안되지만, 
-- -- 휴대폰 번호나 이메일 등은 사람마다 모두 다르기 때문에 UNIQUE로 지정해도 문제 없음
...
-- 인덱스 크기 확인
SHOW TABLE STATUS LIKE 'memeber'; -- LIKE 'member': member라는 글자가 들어간 테이블의 정보 라는 의미
...
-- -- 생성한 인덱스를 실제로 적용시키려면 ANALYZE TABLE 문을 먼저 실행해줘야 함
...
-- > mem_name에 고유 보조 인덱스를 생성하였기 때문에 mem_name에 중복된 값을 입력할 수 없음
-- -- 실제 서비스에서 이름이 중복되어서 회원가입이 안된다면 심각한 문제가 발생할 수 있기 때문에 고유 보조 인덱스는
-- -- 업무상 절대로 중복되지 않는 열에만 생성해야 함
...
-- -- 기본키를 제거하기 전에 외래키 관계를 먼저 제거해야 함
...
-- 1. 인덱스는 열 단위에 생성
-- 2. WHERE절에서 사용되는 열에 인덱스를 생성(보조 인덱스)
-- 3. WHERE절에 사용되더라도 자주 사용될수록 인덱스의 가치가 있음
...
-- 6. 사용하지 않는 인덱스는 제거
-- -- 공간 확보 뿐만 아니라 데이터 입력 시에 발생하는 부하도 줄일 수 있음