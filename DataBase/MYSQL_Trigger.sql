...
-- -- 따라서 member에서 delete 작업이 일어날 경우 데이터가 삭제되기 전에 다른 곳에 자동으로 저장해주는 기능 구현에 사용됨
...
-- 스토어드 프로시저와의 차이점
-- > 트리거는 스토어드 프로시저와 문법이 비슷하지만 CALL로 직접 실행 시킬 수 없고, 오직 테이블에 이벤트가 발생할 경우에만 자동으로 실행됨
...
SELECT @msg;
-- trigger_table에는 DELETE에만 작동하는 트리거를 부착했기 때문에 INSERT문이나 UPDATE문에서는 트리거가 작동하지 않음
...
-- 트리거 활용
-- > 트리거는 테이블에 입력/수정/삭제 되는 정보를 백업하는 용도로 활용할 수 이음
-- > 예) 은행에서 계좌를 만드는 것은 INSERT, 입금이나 출금은 UPDATE, 폐기는 DELETE가 작동한다고 할 때
-- > 누가 계좌를 생성/수정/삭제 했는지 알 수 없다면 계쫘에 문제가 발생했을 때 원인을  파악하기 힘듦
...
( mem_id		CHAR(8) NOT NULL,
  ...
  modDate		Date, -- 변경된 날짜
  modUser		VARCHAR(30) -- 변경된 타입, '수정' 또는 '삭제'
 );
...
-- DELETE문이 실행되면 목표 테이블에서 OLD테이를로 예전값을 잠깐 옮겨둠
