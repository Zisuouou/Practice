...
-- 실제로 저장될 데이터의 형태가 다양하기 때문에 이를 효율적으로 저장하기 위해 위의 데이터 형식에서 세부적으로 다시 여러 데이터 형식으로 나뉨
...
-- VARCHAR가 CHAR보다 메모리 공간을 더 효율적으로 사용할 수 있지만 MYSQL의 처리 속도는 CHAR가 더 빠름
...
(mem_id       CHAR(8) NOT NULL PRIMARY KEY, -- 사용자 아이디(PK)
 ...
 height       TINYINT UNSIGNED -- 부호를 떼는 것 / 평균 키
...
-- 대량의 데이터 형식
-- > CHAR는 최대 255자리까지, VARCHAR는 최대 16383자 까지 지정이 가능
...
-- 데이터 형 변환
-- > 문자형 데이터를 정수형 데이터로 바꾸거나, 정수형 데이터를 문자형 데이터로 바꾸는 등 데이터의 형식을 변환하는 것을 데이터 형 변환(type conversion)이라고 함

-- 형 변환에는 직접 함수를 사용해서 변환하는 명시적 변환(explicit conversion)과 별도의 지시 없이 자연스럽게 변환되는 암식적 변환(implicit conversion)이 있음

-- 함수를 이용한 명시적 변환
-- > 데이터 형식을 변환하는 함수 CASST(), CONVERT()
...
SELECT CAST(AVG(price) AS SIGNE) '평균 가격' FROM buy;
SELECT CONVERT(AVG(price), SIGNED) '평균 가격' FROM buy;

-- 다양한 구분자를 이용한 문자열 데이터를 날짜형으로 변경
SELECT CAST('20204&01&15' AS DATE);
SELECT CAST('20204/01/15' AS DATE);
SELECT CAST('20204%01%15' AS DATE);
SELECT CAST('20204@01@15' AS DATE);
...
-- 만약 길이 조절을 원한다면
SELECT num,
CONCAT(CAST(price AS CHAR(2)), 'X', CAST(amount AS CHAR(1)), '=') '가격x수량', price*amount '구매액'
FROM buy;
