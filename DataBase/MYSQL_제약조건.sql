...
-- > 아이디, 주민등록번호, 이메일 처럼 기본키로 설정할 수 있는 열이 여러 개인 경우에는 테이블의 특성을 가장 잘 반영하는 열을 선택하는 것이 좋음

-- CREATE TABLE에서 기본키 제약조건 설정
USE naver_db;
...
-- > 따라서 구매 테이블의 모든 데이터는 누가 구매했는지 확실하게 알 수 있는 무결성한 데이터가 됨
...
( num 	    INT AUTO_INCREMENT NOT NULL PRIMARY KEY,
 ...
 ALTER TABLE buy -- buy 테이블을 수정
 	ADD CONSTRAINT -- 제약조건을 추가
 	FOREIGN KEY(mem_id) -- 외래키 제약조건을 buy테이블의 mem_id에 설정
...
-- > 하지만 기준 테이블의 열 이름이 변경될 때 참조 테이블의 열 이름이 자동으로 함께 변경된다면더 효율적일 것 임
-- > 즉, 회원 테이블의 BLK가 PLK로 변경되면 자동으로 구매 테이블의 BLK도 PNK로 변경되는 것

-- 위와 같은 기능을 위해 ON UPDATE CASCADE 와 ON DELETE CASCADE를 사용할 수 있음
...
-- -- -- 예) 회원 테이블에 email주소가 있다면 중복되지 않으므로 고유기로 설정할 수 있음
...
INSERT INTO member VALUES('OMG','오마이걸',167,'010');
INSERT INTO member VALUES('OMG','오마이걸',167, NULL);
...
-- -- 기본값이 설정된 열에 기본값을 입력하려면 DEFAULT라고 입력
...
-- NULL값 허용
-- > NULL 값을 허용하려면 생략을 하거나 NULL을 입력하고, 허용하지 않으려면 NOT NULL을 입력
-- > PRIMARY KEY가 설정된 열에는 NULL값이 있을 수 없으므로 생략하면 자동으로 NOT NULL로 인식함
-- > ***NULL은 '아무것도 없다' 라는 의미이므로 공백('')이나 0과는 다름 - 알수없다에 가까움 