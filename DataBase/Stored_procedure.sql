...
-- dnldml 'SQL 문장들'이 한 문장이라면 그 문장만 써도 되지만
-- 두 문장 이상이 처리되어야 한다면 BEGIN ~ END 로 묶어야 함
-- 관습적으로는 한 문장이더라도 나중에 추가될 수 있으니 BEGIN ~ END로 묶음
DROP PROCEDURE IF EXISTS ifProc1; -- 만약에 기존에 ifproc1()을 만든 적이 있다면 삭제
...
-- IF문의 활용
-- > 아이디가 APN인 회원의 데뷔 일자가 5년이 넘었는지 확인해보고 5년이 넘었으면 축하 메세지를 출력
...
BEGIN
	...
	SET point = 88; -- 임의의 값 지정. 실제 데이터가 있다면 SELECT ~ INTO 문 삽입
...
-- > 총 구매액이 0이하(구매한 적 없음)이면 회원등급은 유령고객
...
-- WHILE문의 응용
-- > ITNERATE [레이블] : 지정한 레이블로 돌아가 계속 진행. 파이썬의 continue와 유사
...
BEGIN
	...
	WHILE(i <= 100) DO
		...
		IF (hap > 1000) THEN
			LEAVE mywhile; -- 지정한 label떠남. WHILE문 종료
		END IF;
		SET i = i + 1; -- i의 원래 값에 1을 더해서 다시 i에 넣기
	END WHILE; -- i가 100 이하인 동안에 계속 반복
